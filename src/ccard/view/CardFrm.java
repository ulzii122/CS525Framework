	...
	public CardFrm() {
		...
		for (Customer cust : finco.getCustomerList()) {
			for (IAccount acc : cust.getAccList()) {
				if (acc instanceof GoldCard || acc instanceof BronzeCard || acc instanceof SilverCard) {
					rowdata = new Object[5];
					rowdata[0] = cust.name;
					rowdata[1] = acc.getAccountNum();
					rowdata[2] = cust.city;
					rowdata[3] = (acc instanceof GoldCard ? "Gold" : (acc instanceof SilverCard ? "Silver" : "Bronze"));
